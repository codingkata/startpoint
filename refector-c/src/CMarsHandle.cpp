...
void marsGetCommands(CMarsHandle &cMarsHandle, int &iXmax, int &iYmax,
    ...
    while (std::getline(std::cin, strTmpInput)) {
        ...
        if (iLineInputCur == 1) {            // 表示最大边界输入行
            ...
            if (cMarsHandle.checkPara(strOperation) != 0)
                ...
                iLineInputCur++;
                STParaGroup stUserPara(iXinput, iYinput, cDirectionInput, strOperation);
...
void executeCommands(CMarsHandle &cMarsHandle, int iXmax, int iYmax,
                     std::vector<STParaGroup> &vecUserInput) {
    for (unsigned i = 0; i < vecUserInput.size(); ++i) {
        bool bRip = false;
        int iDcur = cMarsHandle.GetDirection(vecUserInput[i].cDirectionInput); // 当前方向位置
        // 按操作步骤遍历处理
        executeOneCommand(cMarsHandle, iXmax, iYmax, i, vecUserInput, bRip, iDcur);
        // 结果输出
...
void executeOneCommand(CMarsHandle &cMarsHandle, int iXmax, int iYmax, unsigned int i,
                       std::vector<STParaGroup> &vecUserInput, bool &bRip, int &iDcur) {
